...
#define		BQ24150_CURRENT_SHIFT		4
						...
						unsigned long enable_iterm)
{
	u8 no_limit = 3;
    u8 halfamp = 0x01;
    /* set /ce to 0 to enable charger */
    u8 enable_charger = 0;
    u8 vlow = 0x00;

	di->control_reg = ((halfamp << INPUT_CURRENT_LIMIT_SHIFT) | (vlow << VLOW_SHIFT)
				| (enable_iterm << ENABLE_ITERM_SHIFT) | (enable_charger << ENABLE_CHARGER_SHIFT));
						...
						unsigned long voltagemA)
{
    ...
    dev_dbg(di->dev, "Setting voltage to %ld\n", voltagemA);
		...
		unsigned long currentmA, unsigned long term_currentmA)
{
    ...
    dev_dbg(di->dev, "Setting current  to %ld, term current %ld\n", currentmA, term_currentmA);
	if ((di->bqchip_version & (BQ24153 | BQ24158 | BQ24150))) {
...
{
	u8 Vsreg = 2;				/* 160/80 */
    /* BQ24150/1 doesn't have this register */
    if(di->bqchip_version != BQ24150) {
						...
						unsigned long max_voltagemA)
{
    ...
    /* this reg doesn't exist on BQ24150/1 */
    if(di->bqchip_version != BQ24150)
...
static ssize_t bq2415x_show_status(struct device *dev,
				  struct device_attribute *attr,
				  char *buf)
{
    u8 read_reg[5];
    ...
    for (i=0; i < 4; i++){
        read_reg[i]=0;
        ret = bq2415x_read_byte(di, &read_reg[i], (u8) i);
        if (ret < 0) {
            dev_err(&di->client->dev, "Couldn't read CS register\n"); 
        }
    }
    if ((read_reg[0] & 0x30) == 0x00)
        state = "Ready";
    else if ((read_reg[0] & 0x30) == 0x10)
        state = "Charging";
    else if ((read_reg[0] & 0x30) == 0x20)
        state = "Charged";
    else if ((read_reg[0] & 0x30) == 0x30)
        state = "Fault";

    if ((read_reg[1] & 0x04) == 0x00)
        status = "Charger enabled";
    else if ((read_reg[1] & 0x04) == 0x04)
        status = "Charger disabled";

	return sprintf(buf, "%s,%s,%x %x %x %x %x\n", state, status, read_reg[0], 
                                                                    read_reg[1], 
                                                                    read_reg[2], 
                                                                    read_reg[3], 
                                                                    read_reg[4]);
				 ...
				 const struct i2c_device_id *id)
{
	...
	if ((read_reg & 0x18) == 0x08 && (client->addr == 0x6b))
		di->bqchip_version = BQ24150;
...
#endif
    dev_dbg(&client->dev, "BQ chip initialized");
